...
class Processor {
  ...
  static processv2 (rabbit, queue, validationResult) {
    const walletRequest = {
      amount: validationResult.atomicAmount,
      address: validationResult.address,
      confirmations: validationResult.requestConfirmations,
      callback: validationResult.callback,
      callerData: validationResult.callerData,
      privateViewKey: validationResult.privateViewKey
    }
  ...
  static validatev2 (request) {
    const atomicAmount = Helpers.toNumber(request.body.amount)
    const callback = request.body.callback || false
    const address = request.body.address || false
    const privateViewKey = request.body.privateViewKey || false
    const name = request.body.name || false
    const callerData = request.body.userDefined || {}
    const confirmations = Helpers.toNumber(request.body.confirmations)
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!privateViewKey) return reject(new Error('Invalid private view key supplied'))

      const amount = (atomicAmount / Math.pow(10, Config.coinDecimals))

      var decodedAddress
      try {
        decodedAddress = cryptoUtils.decodeAddress(address)
      } catch (e) {
        return reject(new Error('Invalid address supplied'))
      }

      const publicViewKey = cryptoUtils.privateKeyToPublicKey(privateViewKey)

      if (decodedAddress.publicViewKey !== publicViewKey) return reject(new Error('Invalid private view key for the supplied address'))
